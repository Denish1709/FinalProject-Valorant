...
*/
if ( empty( $agent ) || empty($real_name) || empty($origin) || empty($gender) || empty($role) || empty($basic_abilities) || empty($signature_abilities) || empty($ultimate_abilities) || empty($story) || empty($id) ) {
...
$sql = "UPDATE characters SET agent = :agent, real_name = :real_name, origin = :origin, gender = :gender, role = :role, basic_abilities = :basic_abilities, signature_abilities = :signature_abilities, ultimate_abilities = :ultimate_abilities, story = :story, modified_by = :modified_by WHERE id = :id";
